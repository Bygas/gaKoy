  ...
  <div class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
    <div class="game-panel max-w-lg w-full max-h-[80vh] overflow-y-auto border-accent/40">
      ...
      <div v-for="(scene, i) in playedScenes" :key="i" class="mb-3">
        ...
        <p v-if="scene.chosenResponse" class="text-xs text-accent mt-1 ml-2">
      ...
      <div v-if="currentScene">
        ...
        <div v-if="currentScene.choices && !hasChosen" class="space-y-2 mt-3">
          <Button
            v-for="(choice, ci) in currentScene.choices"
            :key="ci"
        ...
        <p v-if="choiceResponse" class="text-xs text-accent mt-2 ml-2">
          → {{ choiceResponse }}
        </p>

        <Button
          v-if="!currentScene.choices || hasChosen"
          class="mt-3 w-full"
          @click="nextScene"
        >
          {{ isLastScene ? 'Bitir' : 'Devam Et' }}
        </Button>
      </div>
    </div>
  </div>
...
const stepTitle = computed(() => {
  if (props.step.phase === 'revealed' && npcDef.value) {
    return `${npcDef.value.name} gerçek suretiyle göründü`
  }

  if (props.step.phase === 'encounter' && npcDef.value) {
    return `${npcDef.value.name} ile karşılaşma`
  }

  return props.step.logMessage ?? 'Gizemli bir alâmet'
})
