            ...
            <div class="border border-accent/10 rounded-xs p-2 mb-2">
              ...
              <template v-if="selectedFriendshipReq">
                <div class="flex items-center justify-between mt-0.5">
                  ...
                  <span
                    class="text-xs"
                    :class="meetsLevel(npcStore.getFriendshipLevel('Mıstık'), selectedFriendshipReq) ? '' : 'text-danger'"
                  >
                    {{ LEVEL_NAMES[npcStore.getFriendshipLevel('Mıstık')] }} / {{ LEVEL_NAMES[selectedFriendshipReq] }}
                  </span>
                </div>
              </template>
            </div>
...
<script setup lang="ts">
  ...
  const canUpgrade = (type: ToolType): boolean => {
    ...
    if (requiredLevel && !meetsLevel(npcStore.getFriendshipLevel('Mıstık'), requiredLevel)) return false
  ...
  const getUpgradeBlockReason = (type: ToolType): string => {
    ...
    const requiredLevel = TIER_FRIENDSHIP_REQ[cost.toTier]
    if (requiredLevel && !meetsLevel(npcStore.getFriendshipLevel('Mıstık'), requiredLevel)) {
