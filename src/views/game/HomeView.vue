  ...
  <div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div v-if="!homeStore.caveUnlocked">
        <p class="text-xs text-muted">Mağara henüz açılmadı. (Toplam kazanç belli bir düzeye ulaşınca kendiliğinden açılır)</p>
      </div>
      <div v-else-if="homeStore.caveChoice === 'none'">
        <p class="text-xs text-muted mb-2">Mağaranın işini seç (bir kez seçilince değişmez):</p>
        <div class="flex flex-col space-y-1">
          ...
          >
            ...
            <span class="text-xs text-muted">Her gün %60 olasılıkla mantar verir</span>
          </div>
          <div
            class="flex items-center justify-between border border-accent/20 rounded-xs px-3 py-1.5 cursor-pointer hover:bg-accent/5"
            @click="handleChooseCave('fruit_bat')"
          >
            <span class="text-xs">Yarasalı Mağara</span>
            <span class="text-xs text-muted">Her gün %50 olasılıkla meyve verir</span>
          </div>
        </div>
      </div>
      <div v-else>
        <p class="text-xs">
          {{ homeStore.caveChoice === 'mushroom' ? 'Mantar Mağarası — her gün yabani mantar çıkma ihtimali vardır.' : 'Yarasalı Mağara — her gün mevsimine göre meyve çıkma ihtimali vardır.' }}
        </p>
      </div>
    </div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div v-if="!homeStore.greenhouseUnlocked">
        <p class="text-xs text-muted mb-2">Sera açılınca her mevsimde ekin yetiştirebilir, ekinleri kendiliğinden sulanmış halde tutarsın.</p>
        <div
          class="flex items-center justify-between border border-accent/20 rounded-xs px-3 py-1.5 cursor-pointer hover:bg-accent/5"
          @click="showGreenhouseModal = true"
        >
          <span class="text-xs">Serayı Aç</span>
          <span class="text-xs text-accent whitespace-nowrap">{{ GREENHOUSE_UNLOCK_COST }} akçe</span>
        </div>
      </div>
      <div v-else>
        <p class="text-xs text-success">Sera açıldı. Çiftlik sekmesinde seraya geçip ekim yapabilirsin.</p>
      </div>
    </div>
    ...
    <div class="border border-accent/20 rounded-xs p-3">
      ...
      <div v-if="!warehouseStore.unlocked">
        <p class="text-xs text-muted mb-2">Depo açılınca eşyaları bölük bölük saklamak için sandık koyabilirsin.</p>
        <div
          class="flex items-center justify-between border border-accent/20 rounded-xs px-3 py-1.5 cursor-pointer hover:bg-accent/5"
          @click="showWarehouseUnlockModal = true"
        >
          <span class="text-xs">Depoyu Aç</span>
          <span class="text-xs text-accent whitespace-nowrap">{{ warehouseStore.UNLOCK_COST }} akçe</span>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div v-else class="flex flex-col items-center justify-center py-4 text-muted mb-2">
          <Warehouse :size="24" />
          <p class="text-xs mt-1">Depo bomboş duruyor</p>
        </div>
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <p class="text-xs text-muted">Açıldıktan sonra her mevsimde ekin yetişir ve ekinler kendiliğinden sulanır.</p>
          </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <p class="text-xs text-muted">Açıldıktan sonra eşyaları türlü türlü sandıklara ayırıp koyabilirsin. Başlangıçta 3 sandık yeri vardır; dükkândan artırılabilir.</p>
          </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2 space-y-1">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs" :class="playerStore.money >= warehouseStore.UNLOCK_COST ? '' : 'text-danger'">
                {{ warehouseStore.UNLOCK_COST }} akçe
              </span>
            </div>
          </div>
      ...
      <div v-if="openChestId" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" @click.self="openChestId = null">
        <div v-if="currentOpenChest" class="game-panel max-w-sm w-full">
          ...
          <div v-else class="flex flex-col items-center justify-center py-6 mb-2">
            ...
            <p class="text-[10px] text-muted/50 mt-0.5">Aşağıdaki “Eşya Koy” düğmesiyle doldurabilirsin</p>
          </div>

          <!-- Tek dokunuşla yinelenen eşya koy -->
          <Button
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full">
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm text-accent">{{ chestQtyModal.mode === 'withdraw' ? 'Al' : 'Koy' }}</p>
          ...
          <Button class="w-full justify-center !bg-accent !text-bg" @click="confirmChestQty">
            {{ chestQtyModal.mode === 'withdraw' ? 'Al' : 'Koy' }} &times;{{ chestQty }}
          </Button>
        </div>
      </div>
    </Transition>
        ...
        <div class="game-panel max-w-xs w-full">
          ...
          <template v-if="dismantleChestInfo">
            <p class="text-xs mb-2">
              “{{ dismantleChestInfo.label }}” adlı sandığı sökmek istediğine emin misin?
              <span v-if="dismantleChestInfo.itemCount > 0" class="text-danger">
                İçindeki {{ dismantleChestInfo.itemCount }} göz eşya heybeye döner.
              </span>
            </p>
            <div class="border border-accent/10 rounded-xs p-2 mb-3">
              <p class="text-[10px] text-muted mb-1">Geri verilecek gereçler (%50)</p>
            ...
            <div class="flex space-x-3 justify-center">
              <Button @click="dismantleChestId = null">Vazgeç</Button>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <div v-if="chestItemDef.sellPrice" class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs text-accent">{{ chestItemDef.sellPrice }} akçe</span>
            </div>
            <div v-if="chestItemDef.staminaRestore" class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs text-success">
                +{{ chestItemDef.staminaRestore }} derman
            ...
            <div v-for="tier in CHEST_TIER_ORDER" :key="tier" class="border border-accent/20 rounded-xs p-2">
              <div class="flex items-center justify-between mb-1">
                <div class="flex items-center space-x-1.5">
                  ...
                  <span class="text-[10px] text-muted">{{ CHEST_DEFS[tier].capacity }} göz</span>
                </div>
                <Button
                  class="py-0 px-1.5"
              ...
              <div class="flex flex-wrap gap-x-3 gap-y-0.5">
                ...
                <span class="text-[10px]" :class="playerStore.money >= CHEST_DEFS[tier].craftMoney ? 'text-muted' : 'text-danger'">
                  {{ CHEST_DEFS[tier].craftMoney }} akçe
                </span>
              </div>
            </div>
          </div>
...
<script setup lang="ts">
  ...
  const handleChooseCave = (choice: 'mushroom' | 'fruit_bat') => {
    if (homeStore.chooseCave(choice)) {
      const name = choice === 'mushroom' ? 'Mantar Mağarası' : 'Yarasalı Mağara'
      addLog(`${name} seçildi; bundan böyle her gün kendi payını verir.`)
    }
  }
  ...
  const handleUnlockFromModal = () => {
    if (homeStore.unlockGreenhouse()) {
      addLog('Sera açıldı! Artık çiftlik sekmesinden seraya geçip her mevsimde ekim yapabilirsin.')
      showGreenhouseModal.value = false
    } else {
      addLog('Akçe ya da gereç yetmediği için sera açılamadı.')
    }
  }
  ...
  const handleUnlockWarehouse = () => {
    ...
    if (!canUnlockWarehouse.value) {
      addLog('Akçe ya da gereç yetmediği için depo açılamadı.')
      return
    }
    ...
    addLog(`Depo açıldı! (-${warehouseStore.UNLOCK_COST} akçe)`)
  }
  ...
  const VOID_ROLES: { value: VoidChestRole; label: string }[] = [
    ...
    { value: 'input', label: 'Girdi Sandığı' },
    { value: 'output', label: 'Çıktı Sandığı' }
  ]
  ...
  /** Heybeden sandığa konabilir eşyalar */
  ...
  /** Tek seferde konabilecek benzer eşyalar */
  ...
  const handleCraftChest = (tier: ChestTier) => {
    ...
    addLog(`${CHEST_DEFS[tier].name} yapıldı! (-${CHEST_DEFS[tier].craftMoney} akçe)`)
  ...
  const confirmDismantle = () => {
    ...
    addLog(`${name} söküldü.${refundText ? ` ${refundText} geri verildi.` : ''}`)
  ...
  const handleSetVoidRole = (chestId: string, role: VoidChestRole) => {
    ...
    const chest = warehouseStore.getChest(chestId)
    if (!chest) return
    if (role === 'none') addLog(`${chest.label} artık görevsiz bırakıldı.`)
    else if (role === 'input') addLog(`${chest.label} girdi sandığı oldu; atölye işlerken gereci önce buradan çeker.`)
    else addLog(`${chest.label} çıktı sandığı oldu; atölyeden çıkan ürün önce buraya düşer.`)
  }
  ...
  const executeChestWithdraw = (chestId: string, itemId: string, quality: Quality, qty: number) => {
    if (!warehouseStore.withdrawFromChest(chestId, itemId, qty, quality)) {
      addLog('Heybe dolu olduğu için eşya alınamadı.')
      return
    }
    addLog(`${getItemName(itemId)}×${qty} alındı.`)
  }
  ...
  const executeChestDeposit = (chestId: string, itemId: string, quality: Quality, qty: number) => {
    ...
    if (actualQty <= 0) {
      addLog('Sandık dolu olduğu için eşya konamadı.')
      return
    }
    addLog(`${getItemName(itemId)}×${actualQty} kondu.`)
  ...
  /** Tek seferde benzer eşyaları koy */
  const handleDepositDuplicates = () => {
    ...
    if (totalDeposited > 0) {
      addLog(`Tek seferde ${kindCount} çeşit eşya, toplam ${totalDeposited} adet kondu.`)
    } else {
      addLog('Sandık dolu olduğu için eşya konamadı.')
    }
  }
